...
func TestInsertUser(t *testing.T) {
	t.Run("Successfully inserts user and returns ID", func(t *testing.T) {
		...
		assert.Equal(t, user.SubscriptionTier, "free")
	})
...
func TestInsertUserWithTier(t *testing.T) {
	t.Run("Successfully inserts user with specified tier", func(t *testing.T) {
		db := newTestDB(t)

		testEmail := "pro@example.com"
		testHashedPassword := "$2a$12$LQv3c1yqBWVHxkd0LHAkCOYz6TtxMQJqhN8/LewrBQ7Q/C0YQxK.6"
		testTier := "pro"

		id, err := db.InsertUserWithTier(testEmail, testHashedPassword, testTier)
		assert.Nil(t, err)
		assert.True(t, id > 0)

		user, found, err := db.GetUser(id)
		assert.Nil(t, err)
		assert.True(t, found)
		assert.Equal(t, user.Email, testEmail)
		assert.Equal(t, user.HashedPassword, testHashedPassword)
		assert.Equal(t, user.SubscriptionTier, testTier)
	})
}

func TestUpdateUserSubscriptionTier(t *testing.T) {
	t.Run("Successfully updates user's subscription tier", func(t *testing.T) {
		db := newTestDB(t)

		originalTier := testUsers["alice"].subscriptionTier
		newTier := "pro"

		err := db.UpdateUserSubscriptionTier(testUsers["alice"].id, newTier)
		assert.Nil(t, err)
		...
		assert.True(t, found)
		assert.Equal(t, newTier, user.SubscriptionTier)
		assert.True(t, user.SubscriptionTier != originalTier)
		assert.Equal(t, user.Email, testUsers["alice"].email)
		...
		assert.Equal(t, user2.SubscriptionTier, testUsers["bob"].subscriptionTier)
		assert.NotEqual(t, user2.SubscriptionTier, user.SubscriptionTier)
	})
	...
	t.Run("Does not error when user ID does not exist", func(t *testing.T) {
		...
		err := db.UpdateUserSubscriptionTier(userID, newTier)
		assert.Nil(t, err)
	})
}
